...
export default function EquipmentSetsPage() {
  ...
  const { data: equipmentSets = [], isLoading, refetch } = useQuery({
  ...
  const handleCreateEquipmentSet = async (data: any) => {
    try {
  ...
  const handleMenuClick = (event: React.MouseEvent<HTMLElement>, setId: string) => {
  ...
  return (
    <Container maxWidth="lg" sx={{ py: 4 }}>
      ...
      >
        <EquipmentSetForm
          onSubmit={handleCreateEquipmentSet}
          title=""
        />
      </Dialog>
