...
try {
  ...
  console.warn(
    'pg not installed; install it to run migrations locally: npm i -D pg',
    e.message
  );
}
...
console.log(
  `- PostgreSQL: ${postgresUrl ? '✅ Configured' : '❌ Not configured'}`
);
...
if (postgresUrl) {
  ...
  console.log(
    'No PostgreSQL database configuration found. Skipping migrations.'
  );
  process.exit(0);
}
...
try {
  ...
  if (isVercel) {
    console.log(
      'Continuing build without migrations (database connection failed)'
    );
    process.exit(0);
  } else {
    process.exit(1);
...
try {
  ...
  for (const file of pendingMigrations) {
    ...
    try {
      ...
      if (
        /already exists|duplicate_object|relation.*already exists/i.test(msg)
      ) {
        ...
        try {
          ...
          await client.query('ROLLBACK');
          console.warn(
            `Could not record ${file} as applied:`,
            recordError.message
          );
        }
        continue;
  ...
  if (isVercel) {
    console.log(
      'Continuing build despite migration failure (Vercel environment)'
    );
